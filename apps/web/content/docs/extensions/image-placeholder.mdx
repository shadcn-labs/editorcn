...
The inserted image ships its own node view: drag the left or right handles to resize it, use the floating controls to change its alignment (left / center / right), and open the more menu to duplicate it, make it full width, or delete it.
...
This installs the node, the node view, the toolbar and `extensions/core/` into your project. It also installs `@tiptap/extension-image`, which the node view uses to insert the final image.
