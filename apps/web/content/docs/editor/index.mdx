...
```bash
npm install @editorcn/editor @tiptap/react @tiptap/pm @tiptap/starter-kit @tiptap/extension-link @tiptap/extension-placeholder @tiptap/extension-text-style @tiptap/extension-color @tiptap/extension-highlight
```
...
A floating formatting menu that appears when you select text inside the content area. It shows inline formatting actions (**bold**, _italic_, underline, strikethrough, code); when the selection is inside a code block it instead shows the syntax language selector. A **color** button opens a single dropdown with two stacked color grids — **Text color** (when `@tiptap/extension-text-style` + `@tiptap/extension-color` are registered) and **Highlight** (when `@tiptap/extension-highlight` is registered with `multicolor: true`) — each with its own remove action. Sections are hidden automatically when the corresponding extension is not registered.
...
Both editors ship a **color control** with text color and highlight. In the toolbar editor it lives inside the bubble menu; in the block editor it is part of `BlockEditor.BubbleMenu`. It renders one button that opens a dropdown with two stacked grids — **Text color** and **Highlight** — each with an 8-color palette and a remove action.

The sections only appear when the matching Tiptap extensions are registered:
...
Both sections show in the dropdown once the extensions are registered. Recolor a word with the highlighter, then use the **remove action** in the dropdown (or the `RichTextEditor.Highlight` control) to strip it.

### Recent colors

Colors you pick are remembered in `localStorage` and shown in a **Recently Used** row at the top of the dropdown (up to 10), so frequently-used text and highlight colors stay one click away.
