...
A horizontal menu anchored to the blinking cursor, vertically centered on the cursor line right after the caret. Unlike the bubble menu (which formats a text selection), the floating menu acts on the block under the caret, so no selection is needed. By default it ships block actions — text, headings 1–3, bullet and ordered lists, quote, code block, divider — and shows only on an empty textblock while the editor is focused. Typing hides it, and selecting text hides it too, so the bubble menu owns selections.

```tsx
import { FloatingMenuExtension } from "@/components/editor";

const editor = useEditor({
  extensions: [
    StarterKit,
    // Optional: tracks the cursor via a ProseMirror plugin.
    // The menu falls back to live selection state without it.
...
<RichTextEditor editor={editor}>
  <RichTextEditor.Toolbar>...</RichTextEditor.Toolbar>
  <RichTextEditor.BubbleMenu editor={editor} />
  <RichTextEditor.FloatingMenu editor={editor} />
  <RichTextEditor.Content />
</RichTextEditor>;
```
...
```tsx
import { defaultFloatingMenuItems } from "@/components/editor";
...
```tsx
import { defaultFloatingMenuShouldShow } from "@/components/editor";
...
The component reads it automatically; an explicit `shouldShow` prop takes precedence. Its visuals follow your shadcn theme tokens and can be customized with the `.rte-floating-menu`, `.rte-floating-group`, and `.rte-floating-item` classes.
