...
The defaults above use **lucide-react**. If your `components.json` uses a
different `iconLibrary`, import the matching preset instead of overriding
icons one by one. Install the library's package first:

| `iconLibrary` | Package(s) | Preset |
| --- | --- | --- |
| `lucide` (default) | `lucide-react` | built-in `DEFAULT_ICONS` |
| `phosphor` | `@phosphor-icons/react` | `phosphorEditorIcons` |
| `tabler` | `@tabler/icons-react` | `tablerEditorIcons` |
| `hugeicons` | `@hugeicons/react` + `@hugeicons/core-free-icons` | `hugeiconsEditorIcons` |
| `remixicon` | `@remixicon/react` | `remixEditorIcons` |
...
You can still override individual keys on top of a preset
(`icons={{ ...phosphorEditorIcons, boldControlIcon: <CustomBold /> }}`).
For files outside `icons.tsx`, run the shadcn CLI migration after install:
`npx shadcn@latest migrate icons --from lucide --to phosphor "src/components/editor/**"`.
...
If your `components.json` uses a different `iconLibrary`, import the matching preset instead. Install the library's package first:

| `iconLibrary` | Package(s) | Preset |
| --- | --- | --- |
| `lucide` (default) | `lucide-react` | built-in `DEFAULT_ICONS` |
| `phosphor` | `@phosphor-icons/react` | `phosphorBlockEditorIcons` |
| `tabler` | `@tabler/icons-react` | `tablerBlockEditorIcons` |
| `hugeicons` | `@hugeicons/react` + `@hugeicons/core-free-icons` | `hugeiconsBlockEditorIcons` |
| `remixicon` | `@remixicon/react` | `remixBlockEditorIcons` |
